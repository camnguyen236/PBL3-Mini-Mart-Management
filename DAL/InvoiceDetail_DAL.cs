...
namespace DAL
{
    public class InvoiceDetail_DAL
    {
        ...
        {
            string query = "select Name_product,ProductGroups.Name_PG,Products.Unit_P, Unit_Price" +
                ", Quantity, Amount from InvoiceDetail inner join Products " +
                "on InvoiceDetail.ID_P = Products.ID_P inner join ProductGroups " +
                "on Products.ID_PG = ProductGroups.ID_PG where ID_Invoice = " + id;
            invoiceDetail = DataProvider.Instance.GetRecords(query);
            return invoiceDetail;
        }
        ...
        {
            string query = "insert into InvoiceDetail(ID_Invoice,ID_P,Unit_Price,Quantity,Name_product) " +
                    "values (N'" + invoiceDetail.ID_Invoice + "',N'" + invoiceDetail.ID_P
                    + "',N'" + invoiceDetail.Unit_Price + "','" + invoiceDetail.Quantity
                    + "',N'" + invoiceDetail.Name_product  + "')";
        ...
        public void updateInvoiceDetail(InvoiceDetail invoiceDetail)
        {
            string query = "update InvoiceDetail set ID_P = N'" + invoiceDetail.ID_P
                ...
                + invoiceDetail.Quantity + "', Name_product = N'" + invoiceDetail.Name_product
        ...
        public DataTable getInvoiceDetailByID(int ID_Invoice)
        {
            ...
            string query = "SELECT Customer.Name_Customer, Customer.Address_Customer" +
                ", Customer.PhoneNumber_Customer, Customer.AccountNumber, Inf_user.Name" +
                ", Invoice.ID_Invoice, Invoice.Invoice_Date, InvoiceDetail.Unit_Price" +
                ", InvoiceDetail.Quantity, InvoiceDetail.Amount, Products.Name_P " +
                "FROM Customer LEFT OUTER JOIN Invoice ON Customer.ID_Customer = Invoice.ID_Customer " +
                "LEFT OUTER JOIN Inf_user ON Invoice.ID = Inf_user.ID LEFT OUTER JOIN InvoiceDetail " +
                "ON Invoice.ID_Invoice = InvoiceDetail.ID_Invoice LEFT OUTER JOIN Products " +
                "ON InvoiceDetail.ID_P = Products.ID_P Where InvoiceDetail.ID_Invoice = " + ID_Invoice;
